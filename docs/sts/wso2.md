...
Assuming that BuckIt server is configured to support STS API by following the doc [BuckIt STS Quickstart Guide](https://docs.min.io/community/minio-object-store/developers/security-token-service.html), execute the following command to temporary credentials from BuckIt server.
...
These credentials can now be used to perform BuckIt API operations, these credentials automatically expire in 1hr. To understand more about credential expiry duration and client grants STS API read further [here](https://github.com/minio/minio/blob/master/docs/sts/client-grants.md).
